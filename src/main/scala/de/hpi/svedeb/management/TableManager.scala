...
object TableManager {
  case class AddTable(name: String, columnNames: List[String])
  ...
  case class TableList(tableNames: List[String])
...
class TableManager extends Actor with ActorLogging {
  ...
  private def addTable(tables: Map[String, ActorRef], name: String, columnNames: List[String]): Unit = {
